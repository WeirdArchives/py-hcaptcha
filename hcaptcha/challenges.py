...
from .utils import random_widget_id, get_captcha_version
from .curves import gen_mouse_move
from urllib.parse import urlparse, urlencode
try:
    from xrequests import Session as HttpClient
except ImportError:
...
class Challenge:
    ...
    def solve(self, answers):
        ...
        resp = self._request(
            method="POST",
            url=f"{self.base_url}/checkcaptcha/{self.key}"
                f"?s={self.sitekey}",
            headers={
                "Host": "hcaptcha.com",
    ...
    def _request(self, method, url, headers=None, data=None, json=None):
        ...
        if resp.status_code == 429:
            raise ApiError(resp.status_code, resp.reason, resp.text)
    ...
    def _check_config(self):
        self._request(
            method="POST",
            url=f"{self.base_url}/checksiteconfig"
                f"?host={self.page.hostname}&sitekey={self.sitekey}&sc=1&swa=1",
            headers={
                "Host": "hcaptcha.com",
                "Connection": "keep-alive",
    ...
    def _get_captcha(self):
        resp = self._request(
            method="POST",
            url=f"{self.base_url}/getcaptcha"
                f"?s={self.sitekey}",
            headers={
                "Host": "hcaptcha.com",
                "Connection": "keep-alive",
    ...
    def _get_proof(self):
        if self._spec["type"] == "hsw":
            return self._get_hsw(self._spec["req"])
        if self._spec["type"] == "hsl":
            return self._get_hsl(self._spec["req"])
        raise HCaptchaError(
            f"Unknown proof type '{self._spec['type']}'")

    def _get_hsw(self, req):
        return temp.get_hsw(req)

    def _get_hsl(self, req):
        return temp.get_hsl(req)
